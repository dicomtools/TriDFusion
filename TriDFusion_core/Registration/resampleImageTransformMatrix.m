...
if strcmpi(atDcmMetaData{1}.StudyInstanceUID, atRefMetaData{1}.StudyInstanceUID) && ...
   bSameOutput == false
    ...
    cand = [ ...
        (dRefFirstZ - dDcmFirstZ) / refSliceThickness, ...
        (dRefFirstZ - dDcmLastZ ) / refSliceThickness, ...
        (dRefLastZ  - dDcmFirstZ) / refSliceThickness, ...
        (dRefLastZ  - dDcmLastZ ) / refSliceThickness  ...
    ];

    [~, idx] = min(abs(cand));
    dFirstOffset = cand(idx);

    % optional: swaps only for debugging/consistency; not strictly needed
    switch idx
        case 2, tmp=dDcmFirstZ; dDcmFirstZ=dDcmLastZ; dDcmLastZ=tmp;
        case 3, tmp=dRefFirstZ; dRefFirstZ=dRefLastZ; dRefLastZ=tmp;
        case 4
            tmp=dDcmFirstZ; dDcmFirstZ=dDcmLastZ; dDcmLastZ=tmp;
            tmp=dRefFirstZ; dRefFirstZ=dRefLastZ; dRefLastZ=tmp;
    end

    dFirstOffset = localChooseBestShiftSign(dFirstOffset, Nz_in, Nz_out);
...
%         Nz_in  = aRspSize(3);
% 
%         % Use your existing function (can be wrong/inverted; we'll robustly fix)
%         [dDcmFirstZ, dDcmLastZ] = getImageZPosition(atDcmMetaData, dcmImage);
%         [dRefFirstZ, dRefLastZ] = getImageZPosition(atRefMetaData, refImage);
% 
%         % --- Pick the endpoint pairing that yields the smallest |offset| in slices ---
%         % candidates are signed offsets (do NOT abs() them)
%         cand = [ ...
%             (dRefFirstZ - dDcmFirstZ) / refSliceThickness, ...
%             (dRefFirstZ - dDcmLastZ ) / refSliceThickness, ...
%             (dRefLastZ  - dDcmFirstZ) / refSliceThickness, ...
...
%         dFirstOffset = cand(idx);
% 
%         % Swap endpoints logically based on which candidate won
%         switch idx
%             case 1
%                 % refFirst vs dcmFirst (no swap)
%             case 2
%                 % refFirst vs dcmLast  => DCM endpoints were inverted
%                 tmp = dDcmFirstZ; dDcmFirstZ = dDcmLastZ; dDcmLastZ = tmp;
%             case 3
%                 % refLast vs dcmFirst  => REF endpoints were inverted
%                 tmp = dRefFirstZ; dRefFirstZ = dRefLastZ; dRefLastZ = tmp;
%             case 4
%                 % refLast vs dcmLast   => both were inverted
%                 tmp = dDcmFirstZ; dDcmFirstZ = dDcmLastZ; dDcmLastZ = tmp;
%                 tmp = dRefFirstZ; dRefFirstZ = dRefLastZ; dRefLastZ = tmp;
%         end
% 
...
%         Nz_in  = aRspSize(3);
% 
%         % dFirstOffset currently from the chosen endpoint pairing (signed)
%         % Now choose its SIGN so the slice shift fills the output best.
%         dFirstOffset = localChooseBestShiftSign(dFirstOffset, Nz_in, Nz_out);
% 
%         dImageOffset = round(dFirstOffset);
%         zMoveOffsetRemaining = dFirstOffset - dImageOffset;
% 
%         % Apply shift (works for Nz_in > Nz_out and Nz_in < Nz_out)
%         fillVal = single(min(resampImage(:)));
%         out = fillVal * ones(aRspSize(1), aRspSize(2), Nz_out, 'single');
% 
%         % mapping: dst = src + dImageOffset
%         srcStart = max(1, 1 - dImageOffset);
%         srcEnd   = min(Nz_in, Nz_out - dImageOffset);
% 
%         if srcEnd >= srcStart
...
%         clear out;
%     end
function off = localChooseBestShiftSign(off, Nz_in, Nz_out)
% Pick +off or -off so that mapping dst = src + round(off)
% yields maximum overlap and minimum padding for the given sizes.

    o1 = round(off);
    o2 = round(-off);

    [ov1, pad1] = localOverlapAndPad(o1, Nz_in, Nz_out);
    [ov2, pad2] = localOverlapAndPad(o2, Nz_in, Nz_out);

    if (ov2 > ov1) || (ov2 == ov1 && pad2 < pad1)
        off = -off;
    end
end

function [overlap, pad] = localOverlapAndPad(o, Nz_in, Nz_out)
% For mapping dst = src + o, compute overlap length and padding amount.

    srcStart = max(1, 1 - o);
    srcEnd   = min(Nz_in, Nz_out - o);

    if srcEnd < srcStart
        overlap = 0;
        pad = Nz_out;   % everything is padding
        return;
    end

    overlap = srcEnd - srcStart + 1;

    dstStart = srcStart + o;
    dstEnd   = srcEnd   + o;

    % how many slices in output are padding (front + back)
    pad = (dstStart - 1) + (Nz_out - dstEnd);
end
