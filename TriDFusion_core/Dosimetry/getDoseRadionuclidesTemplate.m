function tRadionuclide = getDoseRadionuclidesTemplate()
%function tRadionuclide = getDoseRadionuclidesTemplate()
%Retun the Radionuclide Templates from the .mat files of kernel folder.
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    for kk=1:numel(atListing)
        if contains(char(atListing(kk)), '.mat')
            ...
            if strcmpi(sVariable, 'tRadionuclide')  

                load(char(atListing(kk)), 'tRadionuclide');
            end
        end
    end            
end