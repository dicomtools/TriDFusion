function setDoseKernel(sMethod, dModel, sTissue, sIsotope, dKernelKernelCutoffDistance, sKernelInterpolation, bUseCtMap, dCtOffset, bInitDisplay)
%function setDoseKernel(sMethod, dModel, sTissue, sIsotope, dKernelKernelCutoffDistance, sKernelInterpolation, bUseCtMap, dCtOffset, bInitDisplay)
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    BYPASS_CONVOLUTION = false;

    if strcmpi(sMethod, 'Local Deposition')
        USE_LDM_METHOD = true;

    elseif strcmpi(sMethod, 'Kernel Convolution')

        USE_LDM_METHOD = false;
    else % Bypass
        BYPASS_CONVOLUTION = true;
    end
    ...
    if isscalar(atInput) && isFusion('get') == false
        atInput(dSeriesOffset).bFusedDoseKernel = false;
    end
    ...
    aActivity = dicomBuffer('get', [], dSeriesOffset);
...
%                return;
        ...
        if isfield (atCoreMetaData{1}, 'RadiopharmaceuticalInformationSequence')
...
% aTest = aActivity;
    ...
    %            mean(aActivity(62:66,62:66,23:26), 'All')
        ...
        if bUseCtMap == true
            ...
            if isempty(atCtMetaData)
                ...
                atCtMetaData = atInput(tKernelCtDoseMap{dCtOffset}.dSeriesNumber).atDicomInfo;
    ...
    % Add a new series to 3DF

    if BYPASS_CONVOLUTION == false 
        atRtDoseHeader = createRtDoseDicomHeader(aActivity, atCoreMetaData, dSeriesOffset);
    else
        atRtDoseHeader = atCoreMetaData;
        if isfield(atRtDoseHeader{1}, 'RadiopharmaceuticalInformationSequence')      % Set total dose
            
            dSum = sum(aActivity,'all');

            for jj=1:numel(atRtDoseHeader)
                atRtDoseHeader{jj}.RadiopharmaceuticalInformationSequence.Item_1.RadionuclideTotalDose = dSum;
            end       
        end    

        if isfield(atRtDoseHeader{1}, 'RescaleSlope')      % Set total dose


            dTrueMin = min(aActivity, [],'all');
            dTrueMax = max(aActivity, [],'all');
            dTrueRange = dTrueMax-dTrueMin;
            dRescaleSlope = dTrueRange/65535;

            for jj=1:numel(atRtDoseHeader)
                atRtDoseHeader{jj}.RescaleSlope = dRescaleSlope;
            end  
        end
    ...
    atInput(dNewSeriesOffset).bDoseKernel         = bDoseKernel;
    ...
    if BYPASS_CONVOLUTION == false
        ...
        sSeriesDate = atRtDoseHeader{1}.SeriesDate;
        sSeriesTime = atRtDoseHeader{1}.SeriesTime;        
    end
    ...
    aInputBuffer{numel(aInputBuffer)+1} = aActivity;    
    ...
    dicomBuffer('set', aActivity, dNewSeriesOffset);

    setQuantification(dNewSeriesOffset);

    tQuant = quantificationTemplate('get', [], dNewSeriesOffset);


    atInput(dNewSeriesOffset).tQuant = tQuant;

    aMip = computeMIP(aActivity);
    ...
    if exist('acActivityReport', 'var')
        ...
        if ~isempty(acActivityReport)
            ...
            if viewerUIFigure('get') == true
                ...
                dlgMicrosphereReport = ...
                    uifigure('Position', [(getMainWindowPosition('xpos')+(getMainWindowSize('xsize')/2)-DLG_REPORT_X/2) ...
            ...
            setObjectIcon(dlgMicrosphereReport);
