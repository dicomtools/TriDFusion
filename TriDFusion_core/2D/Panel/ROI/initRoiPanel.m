...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if isempty(aBuffer)
        ...
        uicontrol(uiRoiPanelPtr('get'),...
                  ...
                  'position', [15 310 200 20]...
                  );
              
      uiRoiVoiRoiPanel = ...
        uicontrol(uiRoiPanelPtr('get'), ...
                  'Style'   , 'popup', ...
                  'position'  , [95 275 160 20],...
                  'String'  , ' ', ...
                  'Value'   , 1,...
                  'BackgroundColor', viewerBackgroundColor('get'), ...
                  'ForegroundColor', viewerForegroundColor('get'), ...
    ...
    function chkHolesRoiPanelCallback(hObject, ~)
        ...
        if get(chkHolesRoiPanel, 'Value') == true
            set(txtHolesRoiPanel, 'String', 'Contour Holes');            
        else
...
%        set(uiSliderMinTresholdRoiPanel, 'Value', 0);
 ...
 %       uiSliderMinTresholdRoiListener = addlistener(uiSliderMinTresholdRoiPanel, 'Value', 'PreSet', @sliderMinTresholdRoiPanelCallback);  
    ...
    function sliderMaxTresholdRoiPanelCallback(~, hEvent)
        ...
        previewRoiSegmentation(get(chkPixelEdgeRoiPanel, 'Value'), get(chkHolesRoiPanel, 'Value'));
    ...
    function previewRoiSegmentation(bPixelEdge, bHoles)
        ...
        if size(aBuffer, 3) == 1
            ...
            if strcmpi(sActionType, 'Entire Image')
                ...
                if bHoles == true
                    [maskAxe ,~,~,~] = bwboundaries(bwimage(aAxe), 'holes', 4);
                else
                    [maskAxe ,~,~,~] = bwboundaries(bwimage(aAxe), 'noholes', 4);
                end
            ...
            if strcmpi(sActionType, 'Entire Image')
                ...
                if bHoles == true
                    [maskCoronal ,~,~,~] = bwboundaries(bwimage(aCoronal ), 'holes', 4);
                    [maskSagittal,~,~,~] = bwboundaries(bwimage(aSagittal), 'holes', 4);           
                    [maskAxial   ,~,~,~] = bwboundaries(bwimage(aAxial)   , 'holes', 4);                    
                else
                    [maskCoronal ,~,~,~] = bwboundaries(bwimage(aCoronal ), 'noholes', 4);
                    [maskSagittal,~,~,~] = bwboundaries(bwimage(aSagittal), 'noholes', 4);           
                    [maskAxial   ,~,~,~] = bwboundaries(bwimage(aAxial)   , 'noholes', 4);
                end
...
%            set(uiCreateVoiRoiPanel, 'String', 'Cancel');
            ...
            createVoiRoi(bMultipleObjects, bPixelEdge, bHoles);     
        end        
    ...
    function createVoiRoi(bMultipleObjects, bPixelEdge, bHoles)
...
%                        progressBar( aa/aBufferSize, sprintf('Computing slice %d/%d, please wait', bb, aBufferSize) );  
                        ...
                        if aAxial(aAxial==1)
                            ...
                            if bHoles == true
                                [maskAxial,~,~,~] = bwboundaries(aAxial, 'holes', 4);  
                            else
...
%                            end
                        ...
                        if aSlice(aSlice~=0)
                            ...
                            if bHoles == true
                                [maskSlice, ~,~,~] = bwboundaries(aSlice, 'holes', 4); 
                            else
    ...
    function aBW = bwimage(aImage)        
        
        aBW = aImage;
        
        aBW(aBW~=cropValue('get')) = 1;
        aBW(aBW==cropValue('get')) = 0;        
    end
end