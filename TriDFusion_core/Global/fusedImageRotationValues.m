...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    persistent pbApplyRotation;
    persistent paAxe;
    persistent pdRotation;
    
    if strcmpi(sAction, 'set')
        
        pbApplyRotation = bApplyRotationValue;
        
        if exist('aAxeValue', 'var')
            paAxe = aAxeValue;
        end
        if exist('dRotationValue', 'var')
            pdRotation = dRotationValue;
        end        
    end
    
    bApplyRotation = pbApplyRotation;
    aAxe = paAxe;
    dRotation = pdRotation;
end
