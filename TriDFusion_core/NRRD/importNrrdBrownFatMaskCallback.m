function importNrrdBrownFatMaskCallback(~, ~)
%function importNrrdBrownFatMaskCallback()
%Import .nrrd mask file type to TriDFusion.
...
%      [sFileName, sPath] = uigetfile({'*.gz';'*.nii'}, sprintf('%s', char(sCurrentDir), 'Import .nii file'));
     [sFileName, sPath] = uigetfile(sprintf('%s%s', char(sCurrentDir), '*.nrrd'), 'Import .nrrd mask file');
...
%        mainDir('set', sPathName);
        ...
        loadNrrdMaskFile(sPath, sFileName, acLesionMap); 
