function setSegmentationAILiverCallback(~, ~)
%function setSegmentationAILiverCallback()
%Run liver segemntation, The tool is called from the command line.
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    processMachineLearningSegmentation('Liver Segmentation');

    % Export RT-structure
%     writeRTStructCallback();

    atMetaData = dicomMetaData('get', [], dSeriesOffset);
    sOvewriteSeriesDescription = sprintf('RT-%s (Liver)', atMetaData{1}.SeriesDescription);
