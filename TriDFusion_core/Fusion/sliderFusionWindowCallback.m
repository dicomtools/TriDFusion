function sliderFusionWindowCallback(~, ~)
%function sliderFusionWindowCallback(~, ~)
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>. 
    ...
    lMin = fusionWindowLevel('get', 'min');
%       lMax = fusionWindowLevel('get', 'max');
    [lInitMax, ~] = getFusionInitWindowMinMax('get');

    lMax = (1+lInitMax * get(uiFusionSliderWindowPtr('get'),'Value'))/0.5;

    if (lMin < lMax)

        fusionWindowLevel('set', 'max', lMax);
        sliderFusionWindowLevelValue('set', 'max', get(uiFusionSliderWindowPtr('get'),'Value'));
