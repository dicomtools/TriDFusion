function setMachineLearningPSMAGa68ExportToPETCTNetworkCallback(hObject, ~)
%function setMachineLearningPSMAGa68ExportToPETCTNetworkCallback(hObject)
%Export PSMA Ga68 PET\CT series and contours to a AI trainning network, The tool is called from the main menu.
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if isempty(dCTSerieOffset)  
        ...
        progressBar(1, 'Error: PSMA Ga68 PET/CT export to AI network require a CT image!');

        if exist('hObject', 'var')
            errordlg('PSMA Ga68 PET/CT export to AI network require a CT image!', 'Modality Validation');  
        end
        delete(sDFolder);
        return;               
    end
