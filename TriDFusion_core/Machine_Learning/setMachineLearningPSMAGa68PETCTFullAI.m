function setMachineLearningPSMAGa68PETCTFullAI(sPredictScript, tPSMAGa68PETCTFullAI)
%function setMachineLearningPSMAGa68PETCTFullAI(sPredictScript, tPSMAGa68PETCTFullAI)
%Run PSMA Ga68 PET\CT Full AI Segmentation.
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if isempty(dPTSerieOffset) || isempty(dCTSerieOffset)
        progressBar(1, 'Error: PSMA Ga68 full AI segmentation require a PT and CT image!');
        errordlg('PSMA Ga68 full AI segmentation require a PT and CT image!', 'Modality Validation');
        return;
    end
...
%     if tPSMAGa68PETCTFullAI.options.SUVScaled == true
    ...
    if isempty(sNrrdPTFullFileName) || isempty(sNrrdCTFullFileName)
        ...
        if ispc % Windows
            ...
            if bStatus
                ...
                bClassifySegmentation = tPSMAGa68PETCTFullAI.options.classifySegmentation;
                bSmoothMask           = tPSMAGa68PETCTFullAI.options.smoothMask;
                dSmallestValue        = tPSMAGa68PETCTFullAI.options.smallestVoiValue;
                bPixelEdge            = tPSMAGa68PETCTFullAI.options.pixelEdge;
