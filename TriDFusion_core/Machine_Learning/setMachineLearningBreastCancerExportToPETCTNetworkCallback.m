function setMachineLearningBreastCancerExportToPETCTNetworkCallback(hObject, ~)
%function setMachineLearningBreastCancerExportToPETCTNetworkCallback(hObject)
%Export Metastatic Breast Cancer PET\CT series and contours to a AI trainning network, The tool is called from the main menu.
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if isempty(dPTSerieOffset)  
        ...
        progressBar(1, 'Error: Breast Cancer export to AI network require a PT image!');

        if exist('hObject', 'var')
            errordlg('Breast Cancer export to AI network require a PT image!', 'Modality Validation');  
        end
        delete(sDFolder);
        return;               
    end
    ...
    if isempty(dCTSerieOffset)  
        ...
        progressBar(1, 'Error: Breast Cancer PET/CT export to AI network require a CT image!');

        if exist('hObject', 'var')
            errordlg('Breast Cancer PET/CT export to AI network require a CT image!', 'Modality Validation');  
        end
        delete(sDFolder);
        return;               
    end
    ...
    if isempty(atVoi)
        ...
        progressBar(1, 'Error: Breast Cancer export to AI network require contours!');

        if exist('hObject', 'var')
            errordlg('Breast Cancer export to AI network require contours!', 'Contours Validation');  
        end
        delete(sDFolder);
        return;
    end
