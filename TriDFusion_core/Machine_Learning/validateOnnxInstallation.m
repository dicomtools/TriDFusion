function sSegmentatorPath = validateOnnxInstallation()
%function sSegmentatorPath = validateOnnxInstallation()
%Validate machine learning Onnx installation.
...
% along with TriDFusion.  If not, see <http://www.gnu.org/licenses/>.
    ...
    if ispc % Windows
        ...
        [bStatus, sCmdout] = system('WHERE run_onnx_model.py');
        
        if bStatus 
            progressBar( 1, 'Error: Machine learning installation not detected!');
            errordlg(sprintf('Machine learning installation not detected! Please contact Daniel Lafontaine at lafontad@mskcc.org for the installation details.'), 'Machine learning installation validation');  
        else
