function [MP1] = RotationCC(ROOT, nRaios, P2, Raio, Dim, Degree, tamPixel, dND, dField, dNFrames, Hip)
%function [MP] = RotationCC(ROOT, nRaios, P2, Raio, Dim, Degree, tamPixel)
...
% MP = sparse(zeros(nRaios*length(P2),Dim^2));
 ...
 %   addpath('G:\Documents\simind\SytemMatrix')
    ...
    findangle = 1:dNFrames/2;

    g = double(findangle(1):findangle(end)*nRaios);  
...
%     else
%         g = findangle(1):findangle(end)*nRaios;
%     end
...
%     Seq1 = arrayfun(@(x) str2double(num2str(x,40)), Seq1);
    ...
    for i=1:n
        ...
        if mod(i, 10)==1 || i == n % Every 10 to improve speed
            progressBar(i/n, sprintf('Computing 2D system matrix CC rotation step %d/%d.', i, n));
        end
    end
